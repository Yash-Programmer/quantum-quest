...
final predictiveAnalyticsServiceProvider = Provider<PredictiveAnalyticsService>(
  (ref) {
    return PredictiveAnalyticsService();
  },
);

final predictiveNotifierProvider =
    StateNotifierProvider<PredictiveNotifier, PredictiveState>((ref) {
...
class PredictiveNotifier extends StateNotifier<PredictiveState> {
  ...
  Future<void> loadPredictiveData() async {
    ...
    try {
      ...
      state = state.copyWith(predictions: predictions, isLoading: false);
    } catch (e) {
  ...
  Future<void> generateSpendingForecast(List<DataPoint> historicalData) async {
    state = state.copyWith(isLoading: true, error: null);
    try {
      final prediction = await _service.generateSpendingPrediction(
        historicalData,
      );
      ...
      state = state.copyWith(predictions: updatedPredictions, isLoading: false);
  ...
  Future<void> generateCashFlowForecast(
    ...
    state = state.copyWith(isLoading: true, error: null);
    try {
      final prediction = await _service.generateCashFlowPrediction(
        incomeData,
        expenseData,
      );
      final updatedPredictions = [...state.predictions];
  ...
  Future<void> clearPredictions() async {
    ...
    state = state.copyWith(predictions: [], trends: [], insights: []);
  }
  ...
  Future<void> generateDemoData() async {
    ...
    final categoryData = {
      ...
      'Transportation': demoSpendingData
          .map(
            (d) => DataPoint(
              date: d.date,
              value: d.value * 0.7,
          ...
          .toList(),
      'Entertainment': demoSpendingData
          .map(
            (d) => DataPoint(
              date: d.date,
              value: d.value * 0.5,
...
final highImpactInsightsProvider = Provider<List<PredictiveInsight>>((ref) {
  final state = ref.watch(predictiveNotifierProvider);
  return state.insights.where((i) => i.impact > 0.6).toList();
});

final actionableInsightsProvider = Provider<List<PredictiveInsight>>((ref) {
  final state = ref.watch(predictiveNotifierProvider);
  return state.insights.where((i) => i.isActionable).toList();
});
